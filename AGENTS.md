...
### 4.1 CEO Decision Presentation Rule

AIエージェントが篠さん(CEO)の判断を必要とする場合は、確認依頼を次の5項目に整理して提示する。

1. **判断事項**: 今回決める必要があること
2. **現状**: 判断に必要な事実、完了済み事項、制約
3. **選択肢**: 各案と、その影響・トレードオフ
4. **推奨案**: 推奨する選択肢と理由
5. **篠さんが返すべき回答**: そのまま返信できる具体的な回答文

判断依頼は、Gate、ADR、P0方針変更、Production変更、破壊的・不可逆な操作、または選択によって成果が実質的に変わる場合など、既存ルール上でCEO判断が本当に必要な時点に限る。スコープ内の可逆的・低リスクな調査、実装、検査、記録その他の通常作業は、都度確認せず自律的に進める。

この提示ルールは、第3節および他の運用文書が定める承認Gateを免除・緩和するものではない。
